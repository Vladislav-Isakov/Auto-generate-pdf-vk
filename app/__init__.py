...
#функция для обновления токена google, для доступа к таблицам
def update_token_google():
    with open('token.json', 'r+') as file:
        ...
        file.seek(0)
        file.truncate()
        file.write(str(json_token).replace("'", '"'))

#обновление токена каждый день, т.к каждые 3 дня он устаревает
if scheduler.get_job('update_token_google') is None:
    scheduler.add_job(update_token_google, trigger='cron', day='*/1', jitter=60, id='update_token_google', jobstore='default', executor='default', replace_existing=True)
